...
for item in tqdm(data):
        ...
        for idx, reasoning_step in enumerate(item["output"]["all_reasoning_steps"]):
            ...
            if explanation!="":
                ...
                format_data = {
                    ...
                    "lambda_u": np.exp(data_dict["orig_score"]-data_dict["new_score"]).item()
                }
    ...
    for ans in golden_answers:
        if ans in pred:
            save_data_exp += item_list_exp_data
            save_reason_data += reason_data_list
            break

import pdb
pdb.set_trace()
# 整理一下格式，都 save 到 llama-factory 下面
with open("./data/sft_llama3_reason.json", 'w') as f:
    json.dump(save_reason_data, f, ensure_ascii=False, indent=4)            
