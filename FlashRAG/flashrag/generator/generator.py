...
class VLLMHFEXPGenerator(BaseGenerator):
    """Class for decoder-only generator, based on vllm."""
    ...
    def __init__(self, config):
        ...
        self.lora_path = (
            None
            if "explaner_lora_model_path" not in config
            else config["explaner_lora_model_path"]
        )
        self.model_path = config["explaner_model_path"]
        self.use_lora = False
        if self.lora_path is not None:
            self.use_lora = True
        if self.use_lora:
    ...
    def generate(
        ...
        **params,
        ...
        if self.use_lora:
            ...
            outputs = self.model.chat(input_list, sampling_params)
...
class HFEXPGenerator(BaseGenerator):
    """Class for decoder-only generator, based on hf."""
...
class HFPRM(BaseGenerator):
    ...
    def __init__(self, config, model=None):
        ...
        lora_path = (
            None
            if "prm_lora_path" not in config
            else config["prm_lora_path"]
        )
        self.model_path = config["prm_path"]
        self.model, self.tokenizer = self._load_model(model=model)
        self.use_lora = False
        if lora_path is not None:
            self.use_lora = True
