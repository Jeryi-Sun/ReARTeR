...
def get_prm(config, **params):
    model_config = AutoConfig.from_pretrained(config["prm_path"])
    arch = model_config.architectures[0]
    if "t5" in arch.lower() or "bart" in arch.lower():
        return getattr(importlib.import_module("flashrag.generator"), "EncoderDecoderGenerator")(config, **params)
    else:
        return getattr(importlib.import_module("flashrag.generator"), "HFPRM")(config, **params)

def get_explaner(config, **params):
    if config["exp_framework"] == "vllm":
        return getattr(importlib.import_module("flashrag.generator"), "VLLMHFEXPGenerator")(config, **params)
    elif config["exp_framework"] == "openai":
        return getattr(importlib.import_module("flashrag.generator"), "OpenaiHFEXPGenerator")(config, **params)
    else:
        model_config = AutoConfig.from_pretrained(config["explaner_model_path"])
        arch = model_config.architectures[0]
        if "t5" in arch.lower() or "bart" in arch.lower():
            return getattr(importlib.import_module("flashrag.generator"), "EncoderDecoderGenerator")(config, **params)
        else:
            return getattr(importlib.import_module("flashrag.generator"), "HFEXPGenerator")(config, **params)
