...
def _process_tokens(example: dict[str, Any], model: "PreTrainedModel" = None, **kwargs) -> dict:
    ...
    batch = {
        ...
        f"{kwargs['prefix']}score": example["score"],
    }
...
class KTOTrainer(Trainer):
    ...
    def forward(
        ...
        score_list = [batch["score"][i] for i in range(len(batch["label"])) if batch["label"][i] is False]
        if self.calculate_KL:
            KL_logps = None
            KL_model_kwargs = (
                {
        ...
        """
        Add adjust for rejected_logps.
        """
        score_tensor = torch.tensor(score_list, dtype=rejected_logps.dtype, device=rejected_logps.device)
        score_tensor = torch.exp(score_tensor)
        # 检查形状是否匹配，如果 rejected_logps 是高维的，需要调整 score_tensor 的形状
        if rejected_logps.ndim > 1:
            # 假设要在第一维上与 rejected_logps 匹配
            score_tensor = score_tensor.view(-1, *[1] * (rejected_logps.ndim - 1))

        # 对应位置相乘
        rejected_logps = rejected_logps * score_tensor
