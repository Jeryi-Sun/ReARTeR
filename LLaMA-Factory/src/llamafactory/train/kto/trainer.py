...
class CustomKTOTrainer(KTOTrainer):
    ...
    #     r"""
    #     adapt for point-wise reward training
    #     """
    #     labels = inputs.pop("labels", None)
    #     print("++++labels+++++")
    #     print(labels)
    #     score = model(**inputs).logits # shape (batch_size, 1) 取值范围：任意实数
    #     kto_tags = inputs.pop("kto_tags", None) # shape (batch_size, 1) 取值范围：True/False
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        inputs.pop("kl_attention_mask", None)
        inputs.pop("kl_input_ids", None)
        inputs.pop("kl_labels", None)

        labels = inputs.pop("kto_tags", None) # shape (batch_size, 1) 取值范围：True/False
        ...
        labels = labels.float().view(-1, 1) # Reshape to match output shape

        # Compute the loss
        loss = bce_loss_fn(score, labels)
        if is_transformers_version_equal_to_4_46() and kwargs.pop("num_items_in_batch", False):
            if return_outputs:
                return (loss / self.args.gradient_accumulation_steps, score)
            else:
                return loss / self.args.gradient_accumulation_steps
        return loss
    @override
